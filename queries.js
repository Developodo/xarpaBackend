...
*/
  ...
  pool.query(
    ...
    WHERE owner IN (SELECT id_followed FROM test.followers WHERE id_follower=$1 UNION
      SELECT ${owner}) ${where}
    GROUP BY t.id,l.id_users,u.name,u.avatar,t.name,t.distance,t.average,t.timeini,t.pointstart,t.owner,u.avatar,t.image,t.asce,l.id_users,c.id_track,t.atype 
    ORDER BY timeini DESC LIMIT $2 OFFSET $3;`,
    [owner, limit, offset],
    (error, results) => {
      if (error) {
...
getRoutes = (request, response) => {
  ...
  t.xarpa,t.asce,t.desce,l.id_users,t.name,x.image,x.name,c.id_route,t.owner,t.altitude   
